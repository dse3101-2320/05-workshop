...
## Baseball statistics
In the first workshop, we continue working on the `Lahman` data sets on baseball players. Here's the database schema, retrieved from the [Sean Lahman's Baseball Database](https://github.com/cdalzell/Lahman?tab=readme-ov-file).
...
In the second workshop, we will work on data from the [Global Fishing Watch (GFW) project](https://globalfishingwatch.org/).

+ Follow the instructions in the lecture notes and obtain your **project number** from Google Cloud.

+ Store it as an **environment variable**, like with your API keys.
...
fishing_effort <- tbl(gfw_con, "fishing_effort")
```

### Which countries (identified by `flag`) has the longest fishing hours?

```{sql, connection = gfw_con}
...
+ After you finish, remember to disconnect from the database.
